...
def list_image(root, recursive, exts):
    ...
    if recursive:
        ...
        for fname in os.listdir(root):
            ...
            if os.path.isfile(fpath) and (suffix in exts):
                image_list.append(
                    (len(image_list), os.path.relpath(fpath, root), 0))
    return image_list
...
def write_record(args, image_list):
    ...
    def image_encode(item, q_out):
        try:
            img = cv2.imread(
                os.path.join(args.root, item[1]), color_modes[args.color])
        except:
    ...
    try:
        ...
        write_process = multiprocessing.Process(
            target=write_worker, args=(q_out, args.prefix))
        write_process.start()
        for p in read_process:
            p.join()
        q_out.put(('finish', '', []))
...
def main():
    ...
    parser.add_argument(
        'prefix',
        help='prefix of input/output files.')
    parser.add_argument(
        'root',
    ...
    cgroup.add_argument(
        ...
        and create a database at <prefix>.rec')
    cgroup.add_argument(
        '--exts',
        type=list,
        default=['.jpeg', '.jpg'],
    ...
    cgroup.add_argument(
        ...
        type=bool,
        default=False,
        help='If true recursively walk through subdirs and assign an unique\
        label to images in each folder. Otherwise only include images in the\
        root folder and give them label 0.')
    ...
    rgroup.add_argument(
        ...
        from the input list if >1. the input list will be modified to match\
        the resulting order.')
    rgroup.add_argument(
        '--color',
        type=int,
